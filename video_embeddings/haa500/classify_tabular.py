...
def evaluate_and_log(model, X_test, y_test, output_path, model_name):
    ...
    logging.info(f"Saved evaluation metrics and confusion matrix for {model_name}.")
...
def main(args):
    ...
    try:
        # Random Forest
        start = time.time()
        rf, rf_best_params = tune_model(RandomForestClassifier(), {
        ...
        start = time.time()
        evaluate_and_log(rf, X_test, y_test, args.output_path, "RandomForest")
        test_time = time.time() - start

        with open(os.path.join(args.output_path, "RandomForest_best_params.txt"), 'w') as f:
            f.write(str(rf_best_params))

        results.append(["RandomForest", train_time, test_time])

        # XGBoost
        start = time.time()
        xgb, xgb_best_params = tune_model(XGBClassifier(use_label_encoder=False, eval_metric='mlogloss'), {
            'n_estimators': [100, 200],
            'max_depth': [3, 6],
        ...
        start = time.time()
        evaluate_and_log(xgb, X_test, y_test, args.output_path, "XGBoost")
        test_time = time.time() - start

        with open(os.path.join(args.output_path, "XGBoost_best_params.txt"), 'w') as f:
            f.write(str(xgb_best_params))

        results.append(["XGBoost", train_time, test_time])

        # Neural Network
        start = time.time()
        nn, nn_best_params = tune_model(MLPClassifier(max_iter=500), {
        ...
        start = time.time()
        evaluate_and_log(nn, X_test, y_test, args.output_path, "NeuralNet")
        test_time = time.time() - start

        with open(os.path.join(args.output_path, "NeuralNet_best_params.txt"), 'w') as f:
            f.write(str(nn_best_params))

        results.append(["NeuralNet", train_time, test_time])

    except Exception as e:
        logging.exception("Error during training and evaluation")

    # Save summary CSV
    df = pd.DataFrame(results, columns=["Model", "Train Time (s)", "Test Time (s)"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--splits_path', type=str, required=True,
                        help='Path to directory containing train.txt/val.txt/test.txt')
